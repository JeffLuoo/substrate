...
| `gke` | the collector of the GKE managed OTel addon | The addon must be enabled on the cluster. A first install selects this mode on its own when the addon is there. |
| `kind` | the in-cluster collector that the same install applies | The default of a `--kind` install. |

A first install of a cluster that has no `ate-otel-config` ConfigMap tests for the collector of the GKE managed OTel addon, and selects mode `gke` when that collector is there. Thus a cluster which `tools/setup-gcp` prepared, and which has the addon, keeps its telemetry with no flag. This is a test of the cluster, and not an assumption about it: the mode names the Service that the test finds, thus it cannot select a collector that is absent. Give `--observability=none` for no telemetry on such a cluster.
...
```bash
go run ./cmd/ate-setup deploy ate-system --observability=gke   # the cluster is now in mode gke
go run ./cmd/ate-setup deploy atelet                           # keeps mode gke, and reports it
```
